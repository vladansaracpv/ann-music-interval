...
function GenerateIntervalStaticMethods() {
  ...
  const Quality = {
    toAlteration(type: string, quality: string) {
      ...
      if (isDiminished(quality))
        return -1 * either(len, inc(len), eq(type, "P"));
  ...
  const Alteration = {
    toQuality(type: string, alteration: number) {
      ...
      if (lt(alteration, 0))
        return fillStr("d", either(alteration, inc(alteration), eq(type, "P")));
  ...
  const property = (name: string) => (interval: IvlName) => {
    ...
    return ivl.valid ? ivl["name"] : "";
  };

  function build(params: IvlBuild = { octave: 1, direction: 1 }): IvlName {
    let { step, alteration, octave, direction, num } = params;
    if (step !== undefined) num = inc(step) + 7 * dec(octave || 1);
    if (eq(num || -1, -1)) return "";
    if (!isNumber(alteration)) return "";
    let d = lt(direction || 1, 0) ? "-" : "";
    const type = Theory.BASE_INTERVAL_TYPES[dec(Math.abs(num || 1)) % 7];
    return d + num + INTERVAL.Alteration.toQuality(type, alteration);
  }

  function intervalTable(harmonic: number = 0, generic: number = 1) {
    const i0 = { 1: "1P", 2: "2d" };
    ...
    const row = table[harmonic || 0] as Record<number, string>;
    const column = row[generic];
    return column;
  }
  ...
  function addIntervals(i1: IvlName, i2: IvlName, addition = true): IvlName {
    ...
    return interval.valid ? interval.name : "";
  }
...
export function Interval(props: IvlInitProps): IvlProps {
  ...
  const intervalTypeFrom = (harmonic: number, generic: number): string =>
    INTERVAL.intervalTable(harmonic, generic) as string;
  ...
  function createIntervalWithName(interval: IvlName): IvlProps {
    ...
    const alteration = INTERVAL.Quality.toAlteration(
      type,
      quality
    ) as IvlAlteration;
    ...
    const width =
      Theory.BASE_INTERVAL_SIZES[step] + alteration + 12 * dec(octave);
    const semitones = (direction * width) as IvlSemitones;

    // Chroma is position of interval among 12 possible in octave
    const offset =
      (direction * (Theory.BASE_INTERVAL_SIZES[step] + alteration)) % 12;
  ...
  function createIntervalWithSemitones(semitones: IvlSemitones): IvlProps {
    ...
    const tokens = {
      ...
      ...tokenize(_name, Theory.INTERVAL_REGEX)
    };

    const { tn, tq, qn, qq } = tokens;

    if (!tokens) return EmptyInterval;
  ...
  function createIntervalWithNotes(first: string, second: string): IvlProps {
    ...
    const _name = eq(harmonic, 12)
      ? "P8"
      : intervalTypeFrom(harmonic, generic) || "";
  ...
  if (name && IValidators.isIntervalName(name))
    return createIntervalWithName(name);

  if (semitones && isInteger(semitones))
    return createIntervalWithSemitones(semitones);

  if (notes && both(isNoteName(notes[0]), isNoteName(notes[1])))
